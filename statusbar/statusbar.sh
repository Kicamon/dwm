...
# 更新状态栏
refresh() {
  _icons='';_wifi='';_cpu='';_mem='';_date='';_light='';_vol='';_bat='' # 重置所有模块的状态为空
  source $tempfile                                                      # 从 temp 文件中读取模块的状态
  xsetroot -name "$_icons$_wifi$_cpu$_mem$_date$_light$_vol$_bat"       # 更新状态栏
}
...
cron() {
  ...
  while true; do
    to=()                                                               # 存放本次需要更新的模块
      [ $((i % 20)) -eq 0 ]  && to=(${to[@]} wifi)                      # 每 10秒  更新 wifi
      [ $((i % 40)) -eq 0 ]  && to=(${to[@]} cpu mem icons)             # 每 20秒  更新 cpu mem icons
      [ $((i % 60)) -eq 0 ] && to=(${to[@]} bat)                        # 每 30秒 更新 bat
      [ $((i % 1)) -eq 0 ]   && to=(${to[@]} date)                      # 每 0.5秒   更新 date
      [ $i -lt 60 ] && to=(wifi cpu mem date vol light icons bat)       # 前 30秒  更新所有模块
...
case $1 in
  ...
  updateall|check) update icons wifi cpu mem date light vol bat ;;
