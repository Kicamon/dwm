...
#define ISVISIBLE(C)                                                                               \
    ((C->mon->isoverview || C->isglobal || C->tags & C->mon->tagset[C->mon->seltags]))
...
void togglefloating(const Arg *arg) {
    ...
    if (selmon->sel->isfloating) {
        ...
        resize(selmon->sel, selmon->sel->x, selmon->sel->y, selmon->ww / 3 * 2, selmon->wh / 3 * 2,
               0);
    }
...
void tile(Monitor *m) {
    ...
    for (i = 0, my = sy = gappo, c = nexttiled(m->clients); c; c = nexttiled(c->next), i++)
        if (i < m->nmaster) {
            ...
            resize(c, m->wx + mw + gappo, m->wy + sy, m->ww - mw - 2 * c->bw - 2 * gappo,
                   sh - 2 * c->bw, 0);
            sy += HEIGHT(c) + gappi;
        }
}
...
void grid(Monitor *m, uint gappo, uint gappi) {
    ...
    if (n == 2) {
        ...
        ch = (m->wh - 2 * gappo) * 0.65;
        resize(c, m->mx + gappo, m->my + (m->mh - ch) / 2 + gappo, cw - 2 * c->bw, ch - 2 * c->bw,
               0);
